...
// DELETE /api/agent/discussions/[id] — remove discussion + its messages (creator or BUREAU)
export async function DELETE(
  ...
  // Messages first — no guaranteed FK cascade in the live schema
  await supabase.from('AgentDiscussionMessage').delete().eq("discussionId", id);

  const { error } = await supabase.from('AgentDiscussion').delete().eq("id", id);
  if (error) {
