...
export function Select({ label, options, placeholder, className = "", ...props }: SelectProps) {
  return (
    <div className="space-y-1">
      ...
      <select
        className={`w-full px-3 py-2 bg-[#0a0a0c] border border-[rgba(168,144,112,0.08)] text-zinc-300 focus:outline-none focus:border-[rgba(217,119,6,0.2)] transition-all text-xs font-mono min-h-[44px] sm:min-h-auto ${className}`}
        {...props}
      >
